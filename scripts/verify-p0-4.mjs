...
/**
 * One-command P0.4 gate runner (docs/P0_P1_EXECUTION_PLAN_2026-07-10.md §P0.4).
 *
 * Starts one AI_MODE=mock dev server, then runs the P0.4 gate against FRESH
 * seeds (§12.4 — never verify against dirty state):
 *
 *   [seed] -> suite:chat-actions -> teardown -> residue   (the confirmed-chat gate)
...
const PORT = process.env.VERIFY_PORT || "3134";
...
function fail(message) {
  console.error(`\n[verify:p0-4] FAIL CLOSED: ${message}`);
  process.exit(1);
}
...
async function main() {
  ...
  console.log(`[verify:p0-4] starting next dev on ${PORT} (AI_MODE=mock)`);
  ...
  try {
    ...
    await cycle("chat-actions", "scripts/suites/chat-actions.mjs", stepEnv, codes);
    ...
    console.log(
      `\n[verify:p0-4] result — chat-actions:${label(codes["chat-actions"])} integrity:${label(codes.integrity)} ` +
        `e2e:${label(codes.e2e)} teardown:${label(codes["teardown_chat-actions"])}/${label(codes.teardown_integrity)}/${label(codes.teardown_e2e)} ` +
        `residue:${label(codes["residue_chat-actions"])}/${label(codes.residue_integrity)}/${label(codes.residue_e2e)}`
    );
  } catch (error) {
    console.error(`[verify:p0-4] ${error instanceof Error ? error.message : error}`);
    exitCode = 1;
  } finally {
    server.kill("SIGTERM");
  }

  console.log(exitCode === 0 ? "\n[verify:p0-4] P0.4 GATE GREEN ✓" : "\n[verify:p0-4] P0.4 GATE NOT GREEN ✗");
