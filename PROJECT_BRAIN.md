...
- No dedicated `.env.test` Supabase project — **standing owner decision (2026-07-10): "I want everything working in prod."** The harness runs against the production project under the owner-acknowledged regime in `test-isolation.config.json`: mandatory `test_run_id` tagging, teardown after every cycle, residue check as a failing gate. The P0.0 guard (`scripts/test-env.mjs` + `requireServerIsolation()` in the suites, which also verifies the RUNNING server's project via `GET /api/debug/env-fingerprint`) fails closed for any other configuration; provisioning `.env.test` later restores strict isolation automatically. The owner gave standing authorization for tagged suite writes to production for the whole P0/P1 program.
